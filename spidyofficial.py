...
def generate_key(duration):
    ...
    return f"Spidey-{duration.upper()}-{random_part}"
...
def start_command(message):
    ...
    welcome_msg = f"""
<b>💎 Welcome to</b> <code>🔥 Ultimate Attack System 🔥</code>

🧠 <b>Username:</b> @{message.chat.username or 'NoUsername'}  
🆔 <b>User ID:</b> <code>{message.chat.id}</code>

🚫 This bot is private and paid only.  
No free access. No mercy.

📌 <b>Command:</b>
• /price – View premium plans & buy access

<b>⚠️ Warning:</b> Educational & testing use only.  
Misuse is your own risk.

<b>🔐 System Ready. Type</b> <code>/price</code> <b>to continue.</b>
"""
...
def price_command(message):
    response = (
        ...
        "🔣 7𝐃𝐀𝐘 👉 ₹550✔️\n"
        "🔣 30𝐃𝐀𝐘 👉 ₹1600✔️\n\n"
        "Payment here :- https://graph.org/file/44f1b51143b37a16b28b4-fac0619d83cc78682f.jpg\n"
        "Send screenshot to admin fast @SPIDY_OWNEROP"
    )
    bot.reply_to(message, response)
...
def show_recent_logs(message):
    ...
    if user_id in admin_id:
        ...
        response = "⛔️ 𝗔𝗰𝗰𝗲𝘀𝘀 𝗗𝗲𝗻𝗶𝗲𝗱: 𝗔𝗱𝗺𝗶𝗻 𝗼𝗻𝗹𝘆 𝗰𝗼𝗺𝗺𝗮𝗻𝗱 DM TO BUY @SPIDY_OWNEROP"
        bot.reply_to(message, response)
...
def status_command(message):
    ...
    if user_id in admin_id:
        ...
        bot.reply_to(message, "⛔️ 𝗬𝗼𝘂 𝗮𝗿𝗲 𝗻𝗼𝘁 𝗮𝗻 𝗮𝗱𝗺𝗶𝗻 @SPIDY_OWNEROP.")
...
def upload_to_terminal(message):
    """Handle file upload to terminal for admins @SPIDY_OWNEROP."""
    user_id = str(message.chat.id)
...
def add_user_command(message):
    if str(message.chat.id) not in admin_id:
        bot.reply_to(message, "⛔ 𝗔𝗰𝗰𝗲𝘀𝘀 𝗗𝗲𝗻𝗶𝗲𝗱: 𝗔𝗱𝗺𝗶𝗻 𝗼𝗻𝗹𝘆 𝗰𝗼𝗺𝗺𝗮𝗻𝗱 DM TO BUY @SPIDY_OWNEROP DM TO BUY @SPIDY_OWNEROP")
        return
...
def show_modify_options(message):
    if str(message.chat.id) not in admin_id:
        bot.reply_to(message, "⛔ 𝗔𝗰𝗰𝗲𝘀𝘀 𝗗𝗲𝗻𝗶𝗲𝗱: 𝗔𝗱𝗺𝗶𝗻 𝗼𝗻𝗹𝘆 𝗰𝗼𝗺𝗺𝗮𝗻𝗱 DM TO BUY @SPIDY_OWNEROP")
        return
